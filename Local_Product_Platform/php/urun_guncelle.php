...
// Kategori, Üretici ve Ürün güncelleme işlemleri için gerekli sorguları oluşturuyoruz
$categories = [];
$manufacturers = [];
$products = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_product'])) {
        ...
        $sql = "UPDATE urun SET adi = ?, aciklama = ? WHERE urun_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssi", $adi, $aciklama, $urun_id);
...
// Ürünleri al
if (isset($_POST['uretici_id'])) {
    $uretici_id = $_POST['uretici_id'];
    $kategori = $_POST['kategori']; 
    $sql = "SELECT urun_id, adi FROM urun WHERE uretici_id = ? AND kategori = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("is", $uretici_id, $kategori);
    $stmt->execute();
    ...
    <div class="container">
        ...
        <?php if (isset($_POST['uretici_id'])): ?>
            ...
            <form method="POST">
                ...
                <select name="urun_id" id="urun_id" onchange="this.form.submit()">
                    <option value="">--Ürün Seçin--</option>
                    <?php foreach ($products as $product): ?>
                        <option value="<?= $product['urun_id'] ?>" <?= isset($_POST['urun_id']) && $_POST['urun_id'] == $product['urun_id'] ? 'selected' : '' ?>>
                            <?= $product['adi'] ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <?php if (isset($_POST['urun_id'])): ?>
            <!-- Ürün Adı ve Açıklama Güncelleme -->
            <?php
            $urun_id = $_POST['urun_id'];
            $sql = "SELECT adi, aciklama FROM urun WHERE urun_id = ?";
            $stmt = $conn->prepare($sql);
            ...
            <form method="POST">
                ...
                <input type="text" name="adi" id="adi" value="<?= $product['adi'] ?>" required>

                <label for="aciklama">Açıklama:</label>
                <textarea name="aciklama" id="aciklama" required><?= $product['aciklama'] ?></textarea>
